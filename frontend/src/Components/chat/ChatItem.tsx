...
const ChatItem=({content,role,}:{content:string,role:"user" |"assistant";})=> {
    ...
    const auth = useAuth()
  return (
     role==="assistant"? (<Box sx={{display:"flex",p:2,bgcolor:"#004d5612", my:2, gap:2,width: '100%', maxWidth: '73vw',overflowClipBox:"scroll"}}>
        <Avatar sx={{ml:"0"}}>
            <img src="openai.png" alt="openai" width={"30px"}/>
        </Avatar>
        <Box>
         {!messageBlocks && (<Typography sx={{fontSize:"20px",overflowClipBox:"scroll"}}>{content}</Typography>)}
